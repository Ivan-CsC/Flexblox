const DATA_BASE = `PYmt8hd,jdcrLI,jdcrli@gmail.com,27,London
...
DATA_BASE.split('\n').forEach((record) => {
  const [userId, userName, email, age, location] = record.split(',');
  if (!isAlphaNumeric(userId)) {
    console.log(userId + ' no cumple ' + userName);
    result += userName[0];
    return;
  } else {
    console.log(userId + ' cumple ' + userName);
  }

  if (!isOptionalString(location)) {
    console.log(!isOptionalString(location));
    result += userName[0];
    return;
  }

  if (!isOptionalNumber(age)) {
    result += userName[0];
    return;
  }
