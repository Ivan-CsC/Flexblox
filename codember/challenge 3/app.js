...
LIST_POLICES_AND_ENCRYPTIONS.split('\n').forEach((policeAndEncryption) => {
  const [keyPolicy, key] = policeAndEncryption.split(':');
  const keyTrimmed = key.trim();
  const [limits, letter] = keyPolicy.split(' ');
  const [min, max] = limits.split('-');
  const repeats = keyTrimmed.split(letter).length - 1;
  if (repeats < min || repeats > max) {
